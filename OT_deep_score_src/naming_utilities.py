from OT_deep_score_src.general_utilities import FILES_DIR, Model_task, Data_trans_type


##########################################################################
def extract_model_name(model_task, include_distance_feature, include_sequence_features,
                       include_gmt_score, include_nuclea_seq_score, trans_type,
                       trans_all_fold, trans_only_positive, exclude_sg_rnas_without_positives,
                       model_type, fixed_size_encoding):
    """
    extract model name
    """
    ...
    model_name += "-fixEncoding" if fixed_size_encoding else ""
    return model_name
...
def prefix_and_suffix_path(model_task, model_type, data_type, include_distance_feature, include_sequence_features,
    ...
    path_prefix = "{}/{}/{}/{}/".format(data_type, model_task, model_type, trans_type) + path_prefix
...
def extract_model_path(model_task, data_type, include_distance_feature, include_sequence_features,
                       include_gmt_score, include_nuclea_seq_score, trans_type, trans_all_fold,
                       trans_only_positive, exclude_sg_rnas_without_positives, path_prefix,
                       model_type, fixed_size_encoding, bulges, sample_weight,
    ...
    model_path = FILES_DIR + ("bulges/" if bulges else "") + \
        ...
        path_prefix + ("model_fold_{}{}".format(fold_index, sw_txt) if k_fold_number is not None else
                       "model{}".format(sw_txt)) + suffix + ("_fixed" if fixed_size_encoding else "")

    return model_path


# TODO: delete if not in usage
def extract_model_results_path(model_task, data_type, include_distance_feature,
                               ...
                               model_type, fixed_size_encoding, bulges, k_fold_number=None):
    """
    ...
    model_resutls_path = FILES_DIR + "bulges/" if bulges else "" + \
        "{}_folds/".format(k_fold_number) if k_fold_number is not None else "no_folds/" + \
        + path_prefix + "results/" + suffix + ("_fixed" if fixed_size_encoding else "") + ".csv"
